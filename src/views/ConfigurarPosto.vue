  ...
  <v-container fluid class="pa-0  ma-0 fill-height">
    <v-card flat tile class="pa-6 ma-0 fill-height larguratoda" >
        ...
        <v-data-table :headers="headers" :items="filas">
            ...
            <template #item.configurar="{ item }">  
                <v-icon variant="plain"
                class="cursor-pointer mr-2"
                @click="abrirFormConfigurar(item)">mdi-tools</v-icon>
            </template>

            <template #item.visualizar="{ item }">  
                <v-icon variant="plain"
                class="cursor-pointer mr-2"
                @click="abrirForm(item)">mdi-eye-outline</v-icon>
            </template>
        ...
        <FormPosto v-model="openForm"
        :fila="filaSelecionada"
        @salvar="salvarFila"/>
        <FormConfigurar v-model="openForm"
        :fila="filaSelecionada"
        @salvar="salvarFila"/>
    </v-card>
  </v-container>
...
<script setup>
import { ref } from 'vue'
import FormPosto from '../components/Forms/FormPosto.vue'
import FormConfigurar from '../components/Forms/FormPostoConfigurar.vue'
...
const headers = ref([
  ...
  { title: "Configurar", key: "configurar", sortable: false},
  { title: "Editar", key: "editar", sortable: false },
  { title: "Visualizar", key: "visualizar", sortable: false}
])

const filas = ref([
  { nome: "Atendimento 1", status: "", configurar: "", editar: "", visualizar: "" }
]);
...
function abrirFormConfigurar(fila = null) {
    filaSelecionada.value = fila ? {...fila} : null
    openForm.value = true
}
