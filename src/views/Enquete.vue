    ...
    <v-card flat tile class="pa-6 ma-0 fill-height larguratoda" >
        ...
        <v-data-table :headers="headers" :items="filas">
            <template #item.configurar="{ item }">  
                ...
                @click="abrirForm(item)">mdi-cog</v-icon>
            </template>
...
<script setup>
import { ref } from 'vue'
import FormEnquete from '../components/Forms/FormEnquete.vue'

const filaSelecionada = ref(null)
const openForm = ref(false) 

const headers = ref([
  { title: "", key: "configurar" },
  { title: "Enquete", key: "enquete"},
...
const filas = ref([
  { configurar: "", enquete: "Enquete de teste", descricao: "Normal", status: "", editar: "" }
]);
